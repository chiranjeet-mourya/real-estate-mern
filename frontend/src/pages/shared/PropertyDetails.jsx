 ...
 useEffect(() => {
  const fetchPropertyDetails = async () => {
    try {
      ...
      const fetchedProperty = res.data.property;
  ...
  if (!property?.seller?._id) {
    return alert("Seller information is not available.");
  }
  ...
  try {
    ...
    // Backend response: { success: true, chat: {...} }
    const chat = startResponse.data.chat;

    if (!chat?._id) {
      throw new Error("Chat ID not received from server.");
    }
    ...
    await axios.post(
      ...
      {
        chatId: chat._id,
        text: `(Context: interested in this property "${property.title}")`,
  ...
  if (error || !property)
    return (
      ...
      >
        {error ? <p>{error}</p> : <p>Property not found</p>}
      </div>
    );
              ...
              <div className={s.statsGrid}>
                {[
                  ...
                  <div key={i} className={s.statCard}>
                    {stat.icon && <stat.icon className={s.statIcon} />}
            ...
            <div className={s.similarHeader}>
              <div>
                ...
                <p className={s.similarSubtitle}>
                  Listing you mght like in {property.city}.
                </p>
              </div>

              <Link to="/properties" className={s.similaeAllLink}>
                All Listing <HiChevronRight />
              </Link>
            </div>

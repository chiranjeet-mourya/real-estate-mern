  ...
  useEffect(() => {
    ...
    const fetchDashboardData = async () => {
      try {
        ...
        console.log("Faild to load admin dashboard stats:", error);
        setLoading(false);
      }
    };
    fetchDashboardData();
  ...
  return (
    ...
    <div className={s.headerContainer}>
      <div>
        ...
        <p>Welcome back, Administrator. Here's today's summary.</p>
      </div>
      <button onClick={()=>{
        setLoading(true);
        window.location.reload();
      }} className={s.refreshButton}>Refresh Data</button>
    </div>

    <div className={s.statsGrid}>
        {statCards.map((card,i) =>(
            <div key={i} className={s.statCard}>
                <div className={s.statIconContainer} style={{backgroundColor:card.bg, color:card.color}}>
                    <card.icon size={22}/>
                </div>
                <div>

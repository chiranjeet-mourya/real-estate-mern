...
const MyInquiries = () => {
  ...
  useEffect(() => {
    const fetchInquiries = async () => {
      ...
      try {
        ...
        setError(error.response?.data?.message || "Failed to load inquires.");
        setloading(false);
      }
    };
    fetchInquiries();
  ...
  const handleStartChat = async (inq) => {
    try {
      ...
      alert("Failed to start chat. Plaese try again.")
    }
  };
      ...
      <div className={user?.role !== "seller" ? s.bgBgAltMinH : s.bgTransparentMinH}>
        ...
        <div className={s.containerPy12TextCenter}>
          <div className={s.cardPremiumPy16Px8}>
            ...
            <button onClick={()=> window.location.reload()} className={s.btnPrimary}>Try Again</button>
          </div>
        </div>
      </div>
    )
        ...
        >
          <div className={s.mb12}>
            ...
            <p className={s.textMuted}>
              {isSeller ? "Review and respond to intrest in your properties" : "Track the status of your property inquiries."}
            </p>
          </div>
          ...
          {inquiries.length === 0 ? (
            <div className={s.cardPremiumPy24Px8TextCenter}>
              ...
              <p className={s.textMutedMb8}>
                ...
                : "You haven't contacted and seller yet. Interested in a property? send an inquiries!"
              }
              </p>
              ...
              {inquiries.map((inq) =>(
                <div key={inq._id} className={s.inquiryCard}>
                      ...
                      {isSeller && (
                        <div className={s.buyerInfo}>
                          ...
                          <div className={s.infoItem}>
                            ...
                            {inq.buyer?.phone || "No phone number provided"}
                          </div>
                        </div>
                      )}
                  ...
                  <div className={s.actions}>
                    ...
                    {isSeller && (
                      <button onClick={()=> handleStartChat(inq)} className={s.btnMessage}><HiChatAlt2/> Message</button>
                    )}
                  </div>
                </div>
              ))}
